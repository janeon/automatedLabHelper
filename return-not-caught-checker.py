...
class ReturnNotCaught(BaseChecker):
    ...
    def visit_functiondef(self, node):
        ...
        if isinstance(last, astroid.Return):
            self._function_def_stack.append(node.body.name)
            if node.body.name in self._function_calls_stack:
                self.add_message(
                    'return-not-caught', node=node,
                )

    def visit_call(self, node):
        self._function_calls_stack.append(node.body.func.as_string())
        if node.body.func.as_string() in self._function_def_stack:
                self.add_message(
                    'return-not-caught', node=node,
                )
